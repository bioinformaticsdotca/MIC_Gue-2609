...
Leaving the space before the comma empty means that we want to keep all rows, while `1:5` specifies that we want to keep columns 1 through 5.
...
Here, c(1, 3) specifies the rows we want to keep, while leaving the space after the comma empty means that we want to keep all columns.
...
Existing columns can also be replaced with new values. For example, we can multiply every value in flipper_len by two:
...
The `<-` assignment operator is important here. Without it, `pg$flipper_len * 2` would calculate and return the new values but would not modify the original data frame.
...
The write.table() function provides a flexible way to write tabular data to a text file:
...
Because we specified `sep = ","`, the resulting file is a comma-separated values (CSV) file.

R also provides the convenience function write.csv() specifically for writing CSV files:
...
-   To manipulate the aesthetics of a plot using different colors,
    shapes, and lines.
-   To improve data visualization through transforming scales and
    paneling by group.
...
system][base], the [lattice] package, and the [ggplot2] package.
